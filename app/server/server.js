...
Meteor.startup(() => {
  ...
  Meteor.stubServer = new Meteor.StubServer(function() {
    return Meteor.log.info("created stubServer");
  });
  ...
  return this.handleRedisMessage = function(data, callback) {
    ...
    if(message != null && message.header != null && payload != null) {
      ...
      // we explicitly request the users
        ...
        Meteor.log.info(`user ${userId} in meeting ${user.meetingId} is ${validStatus}`);
